...
def show_create_task(w_opts, a_opts):
    ...
    if st.button("🚀 Save and Generate Links", type="primary", use_container_width=True):
        main_p = {
        ...
        if 'id' in main_resp:
            ...
            with tab_wa:
                ...
                st.text_area("WhatsApp Message", value=whatsapp_msg, height=350, key="wa_msg")

            with tab_email:
                st.caption("Copy and paste into your email client:")
                st.text_input("Subject", value=email_subject, key="email_subj")
                st.text_area("Email Body", value=email_body, height=350, key="email_body")
